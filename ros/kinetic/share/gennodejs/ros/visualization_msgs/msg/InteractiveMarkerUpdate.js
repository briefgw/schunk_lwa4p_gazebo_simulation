...
class InteractiveMarkerUpdate {
  ...
  static serialize(obj, bufferInfo) {
    ...
    bufferInfo = _serializer.string(obj.server_id, bufferInfo);
    // Serialize message field [seq_num]
    bufferInfo = _serializer.uint64(obj.seq_num, bufferInfo);
    ...
    obj.markers.forEach((val) => {
      bufferInfo = InteractiveMarker.serialize(val, bufferInfo);
    });
    // Serialize the length for message field [poses]
    bufferInfo = _serializer.uint32(obj.poses.length, bufferInfo);
    // Serialize message field [poses]
    obj.poses.forEach((val) => {
      bufferInfo = InteractiveMarkerPose.serialize(val, bufferInfo);
    });
    // Serialize the length for message field [erases]
    bufferInfo = _serializer.uint32(obj.erases.length, bufferInfo);
    // Serialize message field [erases]
    obj.erases.forEach((val) => {
  ...
  static deserialize(buffer) {
    ...
    for (let i = 0; i < len; ++i) {
      tmp = InteractiveMarker.deserialize(buffer);
    ...
    for (let i = 0; i < len; ++i) {
      tmp = InteractiveMarkerPose.deserialize(buffer);
      data.poses[i] = tmp.data;
      buffer = tmp.buffer;
    }
    // Deserialize array length for message field [erases]
    tmp = _deserializer.uint32(buffer);
    len = tmp.data;
    buffer = tmp.buffer;
    // Deserialize message field [erases]
    data.erases = new Array(len);
    for (let i = 0; i < len; ++i) {
      tmp = _deserializer.string(buffer);
      data.erases[i] = tmp.data;
      buffer = tmp.buffer;
    }
    return {
      data: data,
